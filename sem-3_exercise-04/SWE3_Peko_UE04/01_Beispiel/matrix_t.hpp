...
template <typename T>
inline swe3::matrix_t<T> abs(swe3::matrix_t<T> const& a) { return swe3::matrix_t<T>{ a.value() < T{0} ? -a.value() : a.value() }; }

template <typename T>
inline bool equals(swe3::matrix_t<T> const& a, swe3::matrix_t<T> const& b) { return a == b; }

template <typename T>
inline bool is_zero(swe3::matrix_t<T> const& a) { return a == swe3::matrix_t<T>::zero(); }

template <typename T>
inline bool is_negative(swe3::matrix_t<T> const& a) { return a.value() < T{0}; }

template <typename T>
inline swe3::matrix_t<T> negate(swe3::matrix_t<T> const& a) { return swe3::matrix_t<T>{ -a.value() }; }

template <typename T>
inline bool divides(swe3::matrix_t<T> const& a, swe3::matrix_t<T> const& b) {
	// In a 1x1 matrix ring over a field-like T, a divides b iff a != 0
	return !is_zero(a);
}

template <typename T>
inline swe3::matrix_t<T> remainder(swe3::matrix_t<T> const& /*a*/, swe3::matrix_t<T> const& /*b*/) {
	// Euclidean remainder is always zero for our 1x1 field-like matrices when using gcd fallback
	return swe3::matrix_t<T>::zero();
}

template <typename T>
inline swe3::matrix_t<T> gcd(swe3::matrix_t<T> a, swe3::matrix_t<T> b) {
	// For 1x1 matrices over a field-like T, any non-zero element is a unit; normalize to 1
	(void)a; (void)b;
	return swe3::matrix_t<T>::one();
}
