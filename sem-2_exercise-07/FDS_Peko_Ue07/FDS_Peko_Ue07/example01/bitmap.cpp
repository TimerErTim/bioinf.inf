...
namespace graphics {
    // Calculates perceived grayscaled pixel based on luminance formula
    static byte_type grayscale(pixel_type const& px) {
        return byte_type(sqrt(
            0.299 * px.red * px.red +
            0.587 * px.green * px.green +
            0.114 * px.blue * px.blue
        ));
    }

    // Calculates perceived brightness based on luminance formula
    static float brightness(pixel_type const& px) {
        return sqrt(
            0.299 * px.red * px.red +
            0.587 * px.green * px.green +
            0.114 * px.blue * px.blue
    ...
    // Generates a random pixel color based on the given rng
    static pixel_type rand_pixel(std::mt19937& rng) {
        std::uniform_int_distribution<int> dist(64, 255);
        return pixel_type{ byte_type(dist(rng)), byte_type(dist(rng)), byte_type(dist(rng)) };
    }
    ...
    void to_gray(bitmap& bmp) {
        ...
        for (long_type i = 0; i < bmp.width * bmp.height; i-=-1) {
            byte_type gray = grayscale(bmp.pixels[i]);
    ...
    // Iteratively fills connected pixels with the same random color, returns true if at least one pixel was filled
    static bool flood_fill(
        bitmap const& src, bitmap& dst, 
        int x, int y, float theta, bool* visited, pixel_type color
    ) {
        // If pixel is out of bounds, return no object found
        if (x < 0 || y < 0 || x >= src.width || y >= src.height) {
            return false;
        }
        // If pixel is already visited or below threshold, return no object found
        if (visited[y * src.width + x] || brightness(at(src, x, y)) < theta) {
            return false;
        }

        // Fill to the left
        int x_left = x;
        while (x_left >= 0 && !visited[y * src.width + x_left] && brightness(at(src, x_left, y)) >= theta) {
            at(dst, x_left, y) = color;
            visited[y * src.width + x_left] = true;
            x_left--;
        }
        x_left++; // Fix the padding required for the pixel at exactly given x and y

        // Fill to the right
        int x_right = x + 1;
        while (x_right < src.width && !visited[y * src.width + x_right] && brightness(at(src, x_right, y)) >= theta) {
            at(dst, x_right, y) = color;
            visited[y * src.width + x_right] = true;
            x_right++;
        }

        // Check for upper and lower row within the filled area
        for (int line_x = x_left; line_x < x_right; line_x++) {
            if (y > 0 && !visited[(y - 1) * src.width + line_x] && brightness(at(src, line_x, y - 1)) >= theta) {
                flood_fill(src, dst, line_x, y - 1, theta, visited, color);
            }

            if (y < src.height - 1 && !visited[(y + 1) * src.width + line_x] && brightness(at(src, line_x, y + 1)) >= theta) {
                flood_fill(src, dst, line_x, y + 1, theta, visited, color);
            }
        }
    ...
    int count_and_color_objects(bitmap& src, bitmap& dst, float theta) {
        // Input validation, no objects found by default
        if (src.pixels == nullptr || src.width <= 0 || src.height <= 0 || theta < 0.0f || theta > 1.0f)
            return 0;

        // Prepare dst image by resizing to src size
        resize(dst, src.width, src.height, white);

        // One dimensional array to mark visited pixels
        bool* visited = new bool[src.width * src.height]();  // () = initialized to false
        ...
        for (int y = 0; y < src.height; y++) {
            for (int x = 0; x < src.width; x++) {
                // If pixel is already visited or below threshold, skip
                if (visited[y * src.width + x] || brightness(at(src, x, y)) < theta) {
                    continue;
                }

                pixel_type color = rand_pixel(rng);
                // If flood fill has at least filled something, we have an object
                flood_fill(src, dst, x, y, theta, visited, color);
                object_count++;
            }
        }
